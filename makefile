...
init: build composer-install build-front key-generate storage-link db-migrate seed lunar-install restart build-wait

# a set of commands for initializing a project on production
init-prod: build-prod composer-install build-front key-generate storage-link db-migrate seed lunar-install restart build-prod
...
lunar-install:
	@echo "Lunar install"
	@docker exec -i $$(docker ps -q -f name=php.${APP_NAMESPACE}) php artisan lunar:install -n