            ...
            <CarouselContent>
                {reviews.map((review, idx) => (
                    <CarouselItem key={idx} className="basis-1/2 md:basis-1/3 lg:basis-1/4 xl:basis-1/5">
                        <div className="w-full rounded-lg border p-3 shadow bg-white flex-shrink-0">
                            <div className="flex items-center mb-1">
                                <span className="font-semibold mr-2 text-sm">{review.name}</span>
                                <span className="flex text-yellow-400">
                                    {[1,2,3,4,5].map((star) => (
                                        <svg key={star} xmlns="http://www.w3.org/2000/svg" fill={star <= review.stars ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" className="w-4 h-4">
                            ...
                            <div className="text-gray-700 text-xs mb-1">{review.comment}</div>
